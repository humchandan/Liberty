...
export class ContractService {
  ...
  async getPrimaryAdminFees(): Promise<string> {
    ...
    return ethers.utils.formatUnits(fees, 6);
  }
  ...
  async getReferralInfo(userAddress: string) {
    ...
    return {
      stats: {
        level1Count: info[0][0].toNumber(),
        level2Count: info[0][1].toNumber(),
        level3Count: info[0][2].toNumber(),
        totalEarned: ethers.utils.formatUnits(info[0][3], 6),
      },
      pending: ethers.utils.formatUnits(info[1], 6),
      claimed: ethers.utils.formatUnits(info[2], 6),
      level1: info[3],
    };
  }
   ...
   */
  async payOrder(orderId: number): Promise<ethers.ContractTransaction> {
    const contract = this.getStakingContract();
    return await contract.payOrder(orderId);
  }
  ...
  async getOrderDetails(orderId: number) {
    ...
    return {
      ...
      amount: ethers.utils.formatUnits(order.amount, 6),
      apr: order.apr.toNumber() / 100,
      maturityTimestamp: order.maturityTimestamp.toNumber(),
      paid: order.paid,
  ...
  /**
   * Get pending payout orders (matured but unpaid)
   */
  async getPendingPayoutOrders(): Promise<any[]> {
    const contract = this.getStakingContract();
    const stats = await contract.getPlatformStats();
    const pendingPayouts = stats.pendingPayouts.toNumber();
